...
def color_severity(val):
    if val == 'LOW':
        return 'background-color: #b3e6b3 !important; color: #2d6a2d !important'
    elif val == 'MEDIUM':
...
def load_and_style_job_errors(system, hostname, config):
    base_path = config.get('basePath', '')
    csv_files = config['systems'][system]['files']['csv']
    job_error_files = glob.glob(f"{base_path}/{system}-{hostname}-{csv_files['jobErrors']}")
    
    html_job_errors = ""
    
    for job_error_file in job_error_files:
        df_job_errors = pd.read_csv(job_error_file)
        html_job_errors += df_job_errors.to_html(classes='data-table')
...
for system, system_config in config["systems"].items():
    for instance_config in system_config['instances']:
        ...
        html_job_errors = load_and_style_job_errors(system, hostname, config)
