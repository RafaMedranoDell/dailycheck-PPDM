...
def process_storage_systems(df, system, instance, config):
    ...
    # Save to CSV
    save_dataframe_to_csv(df_storage_systems_output, os.path.join(base_path, f'{system}-{instance}-{csv_files["storageSystems"]}'))
...
def main():
    ...
    for system, system_config in config["systems"].items():
        ...
        for instance_config in system_config['instances']:
            ...
            storage_files = glob.glob(f'{system}-{hostname}-{json_files["storageSystems"]}')
            if not storage_files:
                ...
                for file_path in storage_files:
                    process_if_not_empty(file_path, process_storage_systems, system, hostname, config)
